...
def wilson_lower(successes: pd.Series, trials: pd.Series, z: float = 1.96) -> pd.Series:
    ...
    """
    n  = trials.astype(float).values
    s  = successes.astype(float).values
    z2 = z ** 2
    ...
    p      = s / safe_n
    center = p + z2 / (2 * safe_n)
    margin = z * np.sqrt(p * (1 - p) / safe_n + z2 / (4 * safe_n ** 2))
    denom  = 1 + z2 / safe_n
    lower  = (center - margin) / denom

    # zero out any row where n was actually 0
    lower  = np.where(n == 0, 0.0, lower)
    return pd.Series(np.clip(lower, 0.0, 1.0).round(4), index=trials.index)
